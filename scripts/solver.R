...
eq <- function(L) {
  ...
  s1 <- sum(((1/(sigma*Fc)) * ((sigma/(sigma-1)*d11)^(1-sigma)) * L^(a*gamma2/gamma1) * A^(sigma*(sigma-1)/(2*sigma-1)) * H^((sigma-1)*(sigma-1)*(1-alpha)/(alpha*(2*alpha-1)))))
  s2 <- sum((1/(sigma*Fc)) * ((sigma/(sigma-1)*d11)^(1-sigma)) * L^(a*gamma2/gamma1) * A^(sigma*(sigma-1)/(2*sigma-1)) * H^((sigma-1)*(sigma-1)*(1-alpha)/(alpha*(2*alpha-1))))
  W <- ((L[n]^a * A[n]^((-sigma*(sigma-1)*(sigma-1))/(2*sigma-1)) * H[n]^(-sigma*(sigma-1)*(1-alpha)/(alpha*(2*sigma-1))))/s1)^(1/(1-sigma))
  ...
  cond1 <- L_s^a * 
    A_s^((-sigma*(sigma-1)*(sigma-1))/(2*sigma-1)) * 
    H_s^(-sigma*(sigma-1)*(1-alpha)/(alpha*(2*sigma-1))) - W^(1-sigma) * s1
...
L_sol <- matrix(unlist(nleqslv(init, eq)[1]), 
                nrow = sqrt(regions),ncol = sqrt(regions))
