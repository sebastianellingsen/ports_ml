...
p_load(sandwich, lmtest, AER, stargazer)
...
africa_coast <- africa_coast@data 
#%>% filter(distances_predicted>0)

  m1 <- lm(data=africa_coast, distances_actual ~ distances_predicted + 
             factor(country_var))
  #rse1 <- robust_std(africa_coast$country_var,m1)
  
  m2 <- lm(data=africa_coast, distances_actual ~ distances_predicted+
             factor(country_var) +distance_coast  )
  #rse2 <- robust_std(africa_coast$country_var,m2)
  
  m3 <- lm(data=africa_coast, lights_data ~ distances_predicted+
             factor(country_var) +distance_coast  )
  #rse3 <- robust_std(africa_coast$country_var,m3)
  
  m4 <- lm(data=africa_coast, lights_data ~ distances_actual+
             factor(country_var)  +distance_coast )
  #rse4 <- robust_std(africa_coast$country_var,m4)
  
  iv1 <- ivreg(lights_data ~ distances_actual  +
                 factor(country_var) | distances_predicted + 
                 factor(country_var)  , data=africa_coast)
  #rse5 <- robust_std(africa_coast$country_var,iv1)
  
  iv2 <- ivreg(lights_data ~ distances_actual  +
                 factor(country_var) +distance_coast  + tri | distances_predicted + 
                 factor(country_var)  +distance_coast   + tri, data=africa_coast)
  #rse6 <- robust_std(africa_coast$country_var,iv2)
  
  results_table1 <- stargazer(m1, m2, m3, m4, iv1,iv2, type="text", 
                              ...
                              notes.append = FALSE,
                              model.names = FALSE,
                              omit = c("Constant", "country_var", "distance_coast", "tri"),
